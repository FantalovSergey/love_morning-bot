...
from love_bot.handlers.starting_operations import (
    start_dream_writing, start_dreams_deleting, start_love_messages_deleting,
...
async def test_start_delete_messages_for_Arina(mock_send, message, state):
    message.chat.id = config.ARINA_ID
...
async def test_start_delete_messages(mock_delete, mock_show, message, state):
...
async def test_start_dreams_deleting(
    mock_delete, mock_send, mock_show, message, state,
):
    message.chat.id = config.ARINA_ID
    mock_show.return_value = []
